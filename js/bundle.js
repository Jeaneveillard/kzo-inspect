...
<body>
    ...
    <form class="form-grid" id="form-info">
      ...
      <fieldset>
        ...
        ${(() => {
          ...
          if (!yr) return "";
          const alerts = [];
          if (yr < 1980) alerts.push("⚠️ <strong>Amiante probable</strong> — bardeaux amiante-ciment, plafond popcorn, tuiles 30\xD730, isolant vermiculite \xE0 v\xE9rifier");
          if (yr < 1975) alerts.push("⚠️ <strong>Plomb</strong> — peinture au plomb et tuyaux de plomb possibles");
          if (yr >= 1975 && yr <= 1982) alerts.push("⚠️ <strong>FUUF</strong> — mousse ur\xE9e-formald\xE9hyde possible dans les murs (interdit 1980)");
          if (yr < 1990) alerts.push("⚠️ <strong>Mercure</strong> — thermostats \xE0 ampoule de mercure probables, <strong>BPC</strong> possible (ballasts n\xE9on)");
          if (yr >= 1985 && yr <= 2005) alerts.push("⚠️ <strong>Polybutyl\xE8ne (Poly-B)</strong> — tuyaux gris possible (produit retir\xE9)");
          alerts.push("🔍 <strong>Pyrite</strong> — v\xE9rifier dalle b\xE9ton (zones \xE0 risque QC ind\xE9pendamment de l'ann\xE9e)");
          alerts.push("🔍 <strong>Radon</strong> — test recommand\xE9 si sous-sol habitable");
          return `<div class="form-alert form-alert--warn" style="grid-column:1/-1;padding:0.75rem 1rem;background:#fffbeb;border:1px solid #f59e0b;border-radius:8px;font-size:0.85rem;line-height:1.6">
            <strong>🏠 B\xE2timent ${yr} — \xC9l\xE9ments \xE0 v\xE9rifier selon l'ann\xE9e</strong><br />${alerts.join("<br />")}
          </div>`;
        })()}
  ...
  function autoAddExpert(inspection, contextId, motif) {
    ...
    toast(`R\xE9f\xE9rence ajout\xE9e → ${label}`, "warn");
  }
  ...
  function buildInvoiceHtml(inspection, profile) {
    ...
    const desc = (r.description || 'Inspection préachat résidentielle').replace(/\s*[-—–]\s*(AIBQ|BNQ|IBC|CBQ|REIBH)[^,.]/gi,'').trim();
